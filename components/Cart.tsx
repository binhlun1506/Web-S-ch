              ...
              <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
                ...
                <div className="mt-8">
                  <div className="flow-root">
                    {items.length === 0 ? (
                        <p className="text-center text-slate-400">Giỏ hàng của bạn đang trống.</p>
                    ) : (
                          ...
                          <div className="ml-4 flex flex-1 flex-col">
                            ...
                            <div className="flex flex-1 items-end justify-between text-sm">
                              <div className="flex items-center">
                                <button onClick={() => onUpdateQuantity(item.id, item.quantity - 1)} className="px-2 py-1 border border-slate-600 rounded-l text-slate-300 hover:bg-slate-700">-</button>
                                <p className="w-10 text-center border-t border-b border-slate-600">{`SL: ${item.quantity}`}</p>
                                <button onClick={() => onUpdateQuantity(item.id, item.quantity + 1)} className="px-2 py-1 border border-slate-600 rounded-r text-slate-300 hover:bg-slate-700">+</button>
                              </div>
                              <div className="flex">
                                <button
                                  type="button"
